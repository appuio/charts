...
func Test_ClusterRole_GivenDefaultValues_ThenRenderTemplate(t *testing.T) {
	...
	assert.Equal(t, 2, len(*clusterRoles), "ClusterRole does not have all resources")
	assert.Equal(t, expectedManagerName, (*clusterRoles)[0].Name, "ClusterRole manager does not have correct name")
	assert.Equal(t, expectedMetricsName, (*clusterRoles)[1].Name, "ClusterRole metrics reader does not have correct name")
}
...
func Test_ClusterRole_WhenProxyValues_ThenRenderAllClusterRoles(t *testing.T) {
	...
	assert.Equal(t, 3, len(*clusterRoles), "ClusterRole does not have all resources")
	assert.Equal(t, expectedManagerName, (*clusterRoles)[0].Name, "ClusterRole manager does not have correct name")
	assert.Equal(t, expectedProxyName, (*clusterRoles)[1].Name, "ClusterRole proxy does not have correct name")
	assert.Equal(t, expectedMetricsName, (*clusterRoles)[2].Name, "ClusterRole metrics reader does not have correct name")
}
